---
name: transilience-ideate
description: >-
  Generate Transilience Ideate architecture diagrams and open them via a share URL.
  Use when the user asks for a Transilience Ideate diagram, transilience-ideate board, 3D floor
  stack diagram, architecture sketch to open in Transilience Ideate, or to emit GeneratedGraph
  JSON for Transilience Ideate. Not for Mermaid, Excalidraw, or Mission Control diagram modals.
---

# Transilience Ideate diagrams

Produce a **GeneratedGraph** JSON object, then open it in Transilience Ideate with a `?g=` URL.
Do **not** emit Mermaid, ASCII art, or markdown fences around the JSON when building the URL.

**App origin (production):** `https://transilience-ideate.vercel.app`
...
```bash
ORIGIN="${TRANSILIENCE_IDEATE_ORIGIN:-https://transilience-ideate.vercel.app}"
...
```bash
mkdir -p ~/.claude/skills/transilience-ideate
curl -fsSL https://transilience-ideate.vercel.app/skills/transilience-ideate/SKILL.md \
  -o ~/.claude/skills/transilience-ideate/SKILL.md
```

Or point Claude Code at the folder URL / raw `SKILL.md` and ask it to install into `~/.claude/skills/transilience-ideate/`.
...
- Emit Mermaid for Transilience Ideate (wrong product format).
- Use invented icon names.
- Put more than 48 nodes or hardcode layouts/x,y (Transilience Ideate auto-layouts).
- Use the Mission Control `/api/diagram` Mermaid skill for Transilience Ideate boards.
