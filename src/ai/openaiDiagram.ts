...
const SESSION_KEY = "transilience-ideate:openai-api-key";
...
function promptContext(userPrompt: string, mode: DiagramGenerationMode, currentBoard?: Board): string {
  ...
  const modifyContext = mode === "modify" && currentBoard
    ? `
Current Transilience Ideate graph JSON:
...
`;
  return `You generate Transilience Ideate architecture diagrams.

Return only data that matches the provided JSON schema. Do not include markdown.

Transilience Ideate graph rules:
    ...
    res = await fetch(RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: "json_schema",
            name: "transilience_ideate_generated_graph",
            strict: true,
